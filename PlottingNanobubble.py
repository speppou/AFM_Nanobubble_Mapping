...
f_path = r"F:\Dropbox (Sydney Uni)\PhD\Analysed Data\AFM\Asylum Force Processing\20220407 - Calibrated cantilever re-analysis\20211122- PDMS CVD\adjusted_sens\SHS"
...
for path in glob(os.path.join(f_path, '*.csv')):
    print('path: ' + str(path))
    if os.path.isfile(path) == True and 'Sensitivity' not in path and 'SHS_' not in path:
      ...
      cbar = plt.colorbar(cb, cax=cax1, orientation='horizontal')
      ...
      CB = fig.colorbar(CS3, cax=cax2, orientation='horizontal', ticklocation='top')
#      ax2.set_aspect('equal')

      CB.set_ticks(Levels3)
      CB.set_ticklabels(['0','','4','','8','','12','','16','','20', '150','300','>300'])
      ...
      CB = plt.colorbar(CS3, cax=cax3, orientation='horizontal', ticklocation='top')
      CB.set_ticks(Levels3)
      CB.set_ticklabels(['0','','4','','8','','12','','16','','20', '150','300','>300'])
